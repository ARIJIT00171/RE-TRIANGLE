...
def area_computation_GPU_original(language, video, audio):
...
def area_computation(language, video, audio):
    """
    Compute TRIANGLE area on CPU to avoid GPU OOM for large n.
    language, video, audio: (n, dim) tensors (can be on CUDA when passed).
    Returns: (n, n) tensor on CPU.
    """
    # Detach and move to CPU
    language = language.detach().cpu()
    video = video.detach().cpu()
    audio = audio.detach().cpu()

    with torch.no_grad():
        # language: (n, d)
        language_expanded = language.unsqueeze(1)              # (n, 1, d)
        u = language_expanded - video.unsqueeze(0)             # (n, n, d)
        v = language_expanded - audio.unsqueeze(0)             # (n, n, d)

        u_norm = torch.sum(u ** 2, dim=2)                      # (n, n)
        v_norm = torch.sum(v ** 2, dim=2)                      # (n, n)
        uv_dot = torch.sum(u * v, dim=2)                       # (n, n)

        area = ((u_norm * v_norm) - (uv_dot ** 2)) / 2         # (n, n)
