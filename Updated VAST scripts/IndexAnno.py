...
class AnnoIndexedDataset(Dataset):
    def __init__(self, d_cfg, args):
        ...
        # ---------- filter to only keep ids with BOTH video and audio ----------
        audio_root = getattr(d_cfg, "audio", None)
        vision_root = getattr(d_cfg, "vision", None)
        annos_new = []

        for anno in self.annos:
            # Get some kind of ID (video_id or id)
            vid = anno.get("video_id") or anno.get("id") or anno.get("clip_id")
            if vid is None:
                continue
            ...
            if audio_root is not None:
                ...
                for ext in [".mp3", ".wav", ".m4a"]:
                    if os.path.exists(os.path.join(audio_root, vid + ext)):
                        has_audio = True
                        break

            # Check video exists (try common extensions)
            has_video = True
            if vision_root is not None:
                has_video = False
                for ext in [".mp4", ".mkv", ".webm", ".avi"]:
                    if os.path.exists(os.path.join(vision_root, vid + ext)):
                        has_video = True
                        break
        ...
        if annos_new:
            LOGGER.info(
                f"[{self.dataset_name}] kept {len(annos_new)}/{len(self.annos)} samples with both video+audio."
            )
            self.annos = annos_new
