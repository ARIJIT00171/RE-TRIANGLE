...
def refine_score_matrix(condition_feats, input_ids, attention_mask, score_matrix_t_cond, model, itm_rerank_num, direction='forward'):
    ...
    for i in range(cur_length):
        ...
        for k in range(times):
            slice_input_ids = cur_input_ids[k*small_batch:(k+1)*small_batch].to(device)
            slice_attention_mask = cur_attention_mask[k*small_batch:(k+1)*small_batch].to(device)
            slice_condition_feats = cur_condition_feats[k*small_batch:(k+1)*small_batch].to(device)
