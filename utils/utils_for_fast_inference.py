...
def get_args(pretrain_dir=None):
    class Args:
        ...
        frozen_vision = "no"
        scst_finetuning = "no"
        use_proposal_conv = "yes"
        ret_bidirection_evaluation = "no"
        trainer_type = ""
        itm_rerank_num = 50
        itm_ratio = 1.0
        save_best = "no"
        ...
        use_deepspeed = "no"
        captioner_mode = "no"
        qformer_text_input = "yes"
        evaluate_ret_text = "no"
        pool_vision = "no"
        first_eval = "yes"
        vision_perceiver_query_num = -1
        remove_before_ckpt = "yes"
...
class VisionMapper(object):
    def __init__(self, d_cfg, args):
        ...
        if args.model_cfg.vision_encoder_type.startswith('clip') or args.model_cfg.vision_encoder_type.startswith(
                'evaclip'):
...
class AudioMapper(object):
    ...
    def read(self, audio_path):
        ...
        fbank = torchaudio.compliance.kaldi.fbank(waveform, num_mel_bins=self.melbins, sample_frequency=16000,
        ...
        # #### sample
        output_slices = []
        pad_len = max(self.target_length * self.sample_num - src_length,
                      self.target_length - src_length % self.target_length)
...
def build_batch(args, text, video, audio):
    ...
    for i in range(len(audio)):
        ...
        if audio_frames is not None:
            audio_spectrotriangles.append(audio_frames)
    if audio_spectrotriangles == []:
        return None
    audio_spectrotriangles = torch.stack(audio_spectrotriangles).float()
    ...
    batch['audio_spectrotriangles'] = audio_spectrotriangles.cuda()
