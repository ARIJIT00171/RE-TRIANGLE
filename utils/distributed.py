...
def all_gather_list(data):
    """Gathers arbitrary dataset from all nodes into a list."""
...
def all_gather_list_cpu(data):
    """
    Gathers arbitrary Python objects from all ranks.
    Safe for CPU tensors, lists, dicts.
    """
...
def any_broadcast(data, root_rank):
    """broadcast arbitrary dataset from root_rank to all nodes."""
    enc = pickle.dumps(data)
...
def ddp_allgather_cpu(input):
    """
    All-gather a CPU tensor with variable first dimension.
    Returns concatenated CPU tensor.
    """
    assert not input.is_cuda, "ddp_allgather_cpu expects CPU tensor"

    input = input.contiguous()
    obj_list = [None for _ in range(dist.get_world_size())]
    dist.all_gather_object(obj_list, input)
...
class DistributedSampler_wopadding(DistributedSampler):
    def __iter__(self):
        ...
        # remove tail of dataset to make it evenly divisible.
        if self.drop_last:
