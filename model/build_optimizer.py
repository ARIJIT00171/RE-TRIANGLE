...
def build_optimizer(model, final_config, checkpoint_optim):
    vision_clip = ('vision_encoder_type' in final_config.model_cfg and
                   'clip' in final_config.model_cfg.vision_encoder_type)
    ...
    for k, v in model.named_parameters():
        if any(nd in k for nd in final_config.run_cfg.new_params_name) and not any(nd in k for nd in no_decay):
            new_params.append(v)
            new_params_name.append(k)
        elif any(nd in k for nd in final_config.run_cfg.new_params_name) and any(nd in k for nd in no_decay):
    ...
    optimizer_grouped_parameters = [
        {'params': basic_params, 'weight_decay': final_config.run_cfg.weight_decay,
         'lr': final_config.run_cfg.learning_rate},
        {'params': basic_params_no_decay, 'weight_decay': 0.0, 'lr': final_config.run_cfg.learning_rate},
        {'params': new_params, 'weight_decay': final_config.run_cfg.weight_decay, 'lr': final_config.run_cfg.new_lr},
        {'params': new_params_no_decay, 'weight_decay': 0.0, 'lr': final_config.run_cfg.new_lr},
        {'params': clip_params_visual, 'weight_decay': final_config.run_cfg.weight_decay,
         'lr': final_config.run_cfg.clip_lr},
        {'params': clip_params_no_decay_visual, 'weight_decay': 0.0, 'lr': final_config.run_cfg.clip_lr},
    ]

    if final_config.run_cfg.optim == 'adam':
        optimizer_class = Adam
    elif final_config.run_cfg.optim == 'adamax':
        optimizer_class = Adamax
    elif final_config.run_cfg.optim == 'adamw':
    ...
    for i in optimizer_grouped_parameters:
        i['init_lr'] = i['lr']
    optimizer = optimizer_class(optimizer_grouped_parameters, lr=final_config.run_cfg.learning_rate,
                                betas=final_config.run_cfg.betas)

    optimizer.new_params_name = new_params_name
    optimizer.new_lr = final_config.run_cfg.new_lr
    optimizer.basic_lr = final_config.run_cfg.learning_rate
    optimizer.clip_lr_visual = final_config.run_cfg.clip_lr
