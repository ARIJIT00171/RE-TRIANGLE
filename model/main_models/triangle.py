...
from utils.utils import area_computation_chunked, area_computation_chunked_cos
...
class TRIANGLE(MMGeneralModule):
    ...
    def __init__(self, config, run_cfg):
        ...
        alpha_init = float(getattr(self.config, "area_alpha_init", 0.0))
        self.area_alpha_raw = nn.Parameter(torch.tensor(alpha_init, dtype=torch.float32))
    ...
    def forward_ret(self, batch, task, compute_loss=True, global_step=0):
        ...
        if compute_loss:
            ...
            for task in subtasks:
                #       AREA LOSS COMPUTATION
                #           AREA ITC

                if self.config.model_type == "triangle":
                    area = area_computation_chunked(feat_t, feat_v_all, feat_a_all).clone()
                else:
                    alpha = F.softplus(self.area_alpha_raw)
                    if dist.get_rank() == 0 and global_step % 100 == 0:
                        LOGGER.info(f"Alpha value T2D block: {alpha}")
                    area = area_computation_chunked_cos(feat_t, feat_v_all, feat_a_all, alpha=alpha).clone()
                area = area / self.contra_temp

                if self.config.model_type == "triangle":
                    areaT = area_computation_chunked(feat_t_all, feat_v, feat_a).T.clone()
                else:
                    alpha = F.softplus(self.area_alpha_raw)
                    if dist.get_rank() == 0 and global_step % 100 == 0:
                        LOGGER.info(f"Alpha value D2T block: {alpha}")
                    areaT = area_computation_chunked_cos(feat_t_all, feat_v, feat_a, alpha=alpha).T.clone()
                areaT = areaT / self.contra_temp

                rank = dist.get_rank()
                bs = feat_t.size(0)
                targets = torch.linspace(rank * bs, rank * bs + bs - 1, bs, dtype=int).to(area.device)

                loss = (F.cross_entropy(-area, targets, label_smoothing=0.1)
                        + F.cross_entropy(-areaT, targets, label_smoothing=0.1)) / 2
                ...
                with torch.no_grad():
                    weights_t2cond = F.softmax(-area, dim=1) + 1e-4
                    weights_t2cond[:, rank * bs: rank * bs + bs].fill_diagonal_(0)

                    weights_cond2t = F.softmax(-areaT, dim=1) + 1e-4
                ...
                if dist.get_rank() == 0 and global_step == 100:
                    ...
                    LOGGER.info(f"feat_v requires_grad={feat_v.requires_grad}, grad_fn={feat_v.grad_fn}")
                    LOGGER.info(f"feat_v_all requires_grad={feat_v_all.requires_grad}, grad_fn={feat_v_all.grad_fn}")
                    LOGGER.info(f"feat_a requires_grad={feat_a.requires_grad}, grad_fn={feat_a.grad_fn}")
                    LOGGER.info(f"feat_a_all requires_grad={feat_a_all.requires_grad}, grad_fn={feat_a_all.grad_fn}")
                    LOGGER.info(f"area requires_grad={area.requires_grad}, grad_fn={area.grad_fn}")
                    LOGGER.info(f"loss requires_grad={loss.requires_grad}, grad_fn={loss.grad_fn}")
                    LOGGER.info(f"condition_feats requires_grad={condition_feats.requires_grad}, "
                                f"grad_fn={condition_feats.grad_fn}")
                    LOGGER.info(f"condition_feats_collate requires_grad={condition_feats_collate.requires_grad}, "
                                f"grad_fn={condition_feats_collate.grad_fn}")
                cls = output[:, 0].float()
                ...
                loss_itc.append(torch.zeros((), device=logits.device))
